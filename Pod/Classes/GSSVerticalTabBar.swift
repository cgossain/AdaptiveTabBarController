...
class GSSVerticalTabBar: UIView {
    
    var didSelectItemHandler: ((GSSVerticalTabBarItemView) -> Void)?
    
    var items: [GSSVerticalTabBarItemView]? {
        willSet {
    ...
    override func updateConstraints() {
        if let unwrappedItems = items {
            var previousItem: GSSVerticalTabBarItemView?
            for (idx, item) in unwrappedItems.enumerate() {
    ...
    func tabBarItemTouched(item: GSSVerticalTabBarItemView) {
    ...
    func tabBarItemAtIndex(index: Int) -> GSSVerticalTabBarItemView? {
        var item: GSSVerticalTabBarItemView?
        if let unwrappedItems = items {
